	...
	acf_add_local_field_group(array(
		...
		'fields' => [
			[
				...
				'name' => 'seo_title',
				'type' => 'text',
				'instructions' => '空欄の場合、投稿タイトルが反映されます。文末に「 | CPASSキャリア（シーパスキャリア）」が自動で挿入されます。
',
				'append' => " | {$site_title}",
			],
			[
				...
				'type' => 'textarea',
				'instructions' => '空欄の場合、投稿コンテンツが反映されます。投稿コンテンツがない場合は、サイトディスクリプション（公認会計士をはじめとした高度会計人材のための転職支援サービス。実務経験の長いキャリアコンサルタントと、公認会計士のサポーターが協力しながら、あなたの転職をご支援いたします。会計人材のインフラ企業だからこそ集まる、ハイクラスな求人情報を多数ご紹介。）が反映されます。<br>*70~120文字推奨',
			],
		],
		'location' => array(
			array(
...
function vanilla_set_default_seo_title($post_id) {
	...
	// 投稿タイプが存在しない場合や、自動保存の場合は何もしない
	if (!$post->post_type || (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE)) {
		return;
	}

	// 投稿ステータスが 'publish' または 'draft' の場合のみ実行
	if (isset($post->post_status) && !in_array($post->post_status, array('publish', 'draft'), true)) {
		return;
	}
	...
	if (!$seo_title) {
		...
		if ($post->post_type === 'consultant') {
			...
			update_field('seo_title', $consultant_name, $post_id);
		}
		//== 投稿タイプ「 転職成功事例 」 ====
		elseif ($post->post_type === 'case') {
			update_field('seo_title', "転職成功事例 {$post_title}", $post_id);
		}
		//== その他 ====
		else {
	...
	//========================
	// seo_title が存在していて、かつ「｜CPASSキャリア（シーパスキャリア）」が含まれている場合
	//========================
	elseif ($seo_title && strpos($seo_title,$site_title) !== false){
    $new_title = str_replace("｜{$site_title}", '', $seo_title);
		update_field('seo_title', $new_title, $post_id);
	}
}
add_action('wp_insert_post', 'vanilla_set_default_seo_title');
...
add_action('wp_insert_post', 'vanilla_set_default_seo_description');
